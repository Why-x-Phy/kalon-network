...
# Testet: Transaction Signature Validation, Merkle Root, Fork-Erkennung, Difficulty Validation
...
DATA_DIR="data-complete-all-features-test"
RPC_PORT=16316
NODE_PID=""
MINER_PID=""
...
if grep -qi "difficulty" "$DATA_DIR/node.log" | grep -v "DEBUG" | head -5; then
    echo -e "${GREEN}✅ Difficulty-related messages found${NC}"
else
    echo -e "${YELLOW}⚠️ No difficulty validation messages (checking for errors)${NC}"
fi
echo ""
...
# Summary
echo "=== TEST SUMMARY ==="
echo -e "${GREEN}✅ Complete All Features Test Completed${NC}"
echo "Blocks mined: $BLOCKS_MINED"
