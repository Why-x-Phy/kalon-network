...
```
Type: A
Name: node (oder api, rpc, etc.)
Value: 185.133.249.107
TTL: 3600 (oder Auto)
```

**Ergebnis:**
- `node.kalon-network.de` → `185.133.249.107`
- Oder: `api.deine-domain.de` → `185.133.249.107`

**Wichtig:** Warte 5-15 Minuten bis DNS propagiert ist. Prüfe mit:
```bash
dig node.kalon-network.de
# Oder:
nslookup node.kalon-network.de
```
...
sudo certbot certonly --standalone -d node.kalon-network.de

# Zertifikate finden:
# → /etc/letsencrypt/live/node.kalon-network.de/fullchain.pem
# → /etc/letsencrypt/live/node.kalon-network.de/privkey.pem
```
...
./build-v2/kalon-node-v2 \
  ...
  -rpc 0.0.0.0:16316 \
  -https 0.0.0.0:16317 \
  -certfile /etc/letsencrypt/live/node.kalon-network.de/fullchain.pem \
  -keyfile /etc/letsencrypt/live/node.kalon-network.de/privkey.pem \
  -p2p 0.0.0.0:17335 \
  > node.log 2>&1 &
```
...
if (typeof window.RPC_URL === 'undefined') {
    window.RPC_URL = 'https://node.kalon-network.de:16317';
}
```

**Wichtig:** Ersetze `node.kalon-network.de` mit deiner tatsächlichen Subdomain!

## Schritt 6: Testen

```bash
# HTTPS testen (sollte funktionieren ohne -k):
curl https://node.kalon-network.de:16317/rpc \
...
sudo crontab -e
# Füge hinzu:
0 0 * * * certbot renew --quiet --deploy-hook "systemctl restart kalon-node"
```
...
✅ **DNS A-Record:** `node.deine-domain.de` → `185.133.249.107`
✅ **Certbot:** Let's Encrypt Zertifikat auf Node-Server
✅ **Node:** Start mit Let's Encrypt Zertifikat
✅ **Explorer:** Ruft `https://node.deine-domain.de:16317` auf
✅ **Keine Browser-Warnung!**
