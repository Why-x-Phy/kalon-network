...
# Vollumfänglicher Komplett-Test für alle Features
# Testet: Node, Wallet, Mining, Transactions, Signature Validation, Merkle Root, Fork-Erkennung
...
DATA_DIR="data-complete-fork-test"
RPC_PORT=16316
NODE_PID=""
MINER_PID=""
...
echo "" | ./build-v2/kalon-wallet send -name wallet1 -to "$WALLET2_ADDRESS" -amount 1000000 -rpc http://localhost:$RPC_PORT > "$DATA_DIR/tx_output.txt" 2>&1 || true
TX_OUTPUT=$(cat "$DATA_DIR/tx_output.txt")
echo "$TX_OUTPUT"

if echo "$TX_OUTPUT" | grep -qi "success\|submitted\|sent"; then
...
# Get updated best block
echo "=== GET UPDATED BEST BLOCK ==="
BEST_BLOCK_RESPONSE2=$(curl -s -X POST -H "Content-Type: application/json" \
    -d '{"jsonrpc":"2.0","method":"getBestBlock","params":[],"id":1}' \
    http://localhost:$RPC_PORT)

BEST_BLOCK_NUMBER2=$(echo "$BEST_BLOCK_RESPONSE2" | grep -oP '"number":\s*\K[0-9]+' || echo "0")
BEST_BLOCK_MERKLE2=$(echo "$BEST_BLOCK_RESPONSE2" | grep -oP '"merkleRoot":\s*"0x\K[0-9a-f]+' || echo "")

echo "Updated Best Block Number: $BEST_BLOCK_NUMBER2"
echo "Updated Best Block Merkle Root: $BEST_BLOCK_MERKLE2"
echo ""
...
if grep -q "Reorganizing chain\|Chain reorganization" "$DATA_DIR/node.log"; then
    ...
    grep "Reorganizing chain\|Chain reorganization" "$DATA_DIR/node.log" | tail -5
else
...
if grep -qi "signature\|validation" "$DATA_DIR/node.log" | grep -v "DEBUG" | head -5; then
    echo -e "${GREEN}✅ Signature validation messages found${NC}"
else
    echo -e "${YELLOW}⚠️ No signature validation messages (checking for errors)${NC}"
fi
echo ""
...
# Summary
echo "=== TEST SUMMARY ==="
echo -e "${GREEN}✅ Complete Fork Detection Test Completed${NC}"
echo "Blocks mined: $BLOCKS_MINED"
