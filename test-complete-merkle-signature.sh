...
# Vollumfänglicher Komplett-Test für Transaction Signature Validation + Merkle Root
# Testet: Node, Wallet, Mining, Transactions, Signature Validation, Merkle Root
...
cleanup() {
    ...
    sleep 2
    rm -rf data-complete-test 2>/dev/null || true
...
TEST_DIR="data-complete-test"
rm -rf "$TEST_DIR" 2>/dev/null || true
mkdir -p "$TEST_DIR"

log "=========================================="
log "  VOLLUMFÄNGLICHER KOMPLETT-TEST"
log "  Transaction Signature + Merkle Root"
log "=========================================="
echo ""
...
for i in {1..90}; do
    ...
    if [ -n "$HEIGHT" ] && [ "$HEIGHT" != "null" ] && [ "$HEIGHT" -gt 5 ]; then
        BLOCKS_MINED=$HEIGHT
    ...
    if [ $i -eq 90 ]; then
        warning "Nur $HEIGHT Blöcke nach 90 Sekunden gemined"
        BLOCKS_MINED=$HEIGHT
        break
    fi
...
# Schritt 8: Transaction senden (mit Signature Validation)
log "Schritt 8: Sende Transaction von Wallet 1 zu Wallet 2 (Signature Validation Test)..."
sleep 3
...
if [ -n "$TX_ERROR" ] && [ "$TX_ERROR" != "None" ] && [ "$TX_ERROR" != "" ]; then
    if echo "$TX_ERROR" | grep -qi "insufficient\|balance"; then
        ...
        success "✅ Transaction Signature Validation funktioniert (Transaction wurde akzeptiert)"
    else
...
# Schritt 9: Prüfe mehrere Blöcke auf Merkle Root
...
# Schritt 10: Prüfe Node-Logs auf Fehler
log "Schritt 10: Prüfe Node-Logs auf Fehler..."
ERROR_COUNT=$(grep -i "error\|failed\|invalid\|merkle.*invalid\|signature.*invalid" node.log 2>/dev/null | grep -v "DEBUG" | wc -l || echo "0")

if [ "$ERROR_COUNT" -gt 0 ]; then
    warning "Gefundene Fehler in Node-Logs: $ERROR_COUNT"
    log "Relevante Fehler:"
    grep -i "error\|failed\|invalid\|merkle.*invalid\|signature.*invalid" node.log 2>/dev/null | grep -v "DEBUG" | tail -5
else
    success "✅ Keine Fehler in Node-Logs gefunden"
fi
echo ""

# Schritt 11: Prüfe Miner-Logs auf Fehler
log "Schritt 11: Prüfe Miner-Logs auf Fehler..."
MINER_ERROR_COUNT=$(grep -i "error\|failed\|invalid" miner.log 2>/dev/null | wc -l || echo "0")

if [ "$MINER_ERROR_COUNT" -gt 0 ]; then
    warning "Gefundene Fehler in Miner-Logs: $MINER_ERROR_COUNT"
    log "Relevante Fehler:"
    grep -i "error\|failed\|invalid" miner.log 2>/dev/null | tail -5
else
    success "✅ Keine Fehler in Miner-Logs gefunden"
fi
echo ""

# Schritt 12: Finale Statistiken
log "Schritt 12: Finale Statistiken..."
success "✅ Finale Block-Höhe: $FINAL_HEIGHT"
success "✅ Wallet 1 Adresse: $WALLET1_ADDRESS"
success "✅ Wallet 2 Adresse: $WALLET2_ADDRESS"
success "✅ Blöcke mit Merkle Root: $BLOCKS_WITH_MERKLE / $BLOCKS_CHECKED"
...
success "✅ Alle Features funktionieren zusammen"
echo ""
